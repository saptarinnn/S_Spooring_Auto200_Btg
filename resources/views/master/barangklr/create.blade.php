<x-app-layout title="Dashboard | Spooring Auto2000 Bontang">
    <section class="container px-4 pb-10 mx-auto">
        ...
        <x-master.card>
            <x-master.form action="{{ route('brg-keluar.store') }}">
                ...
                <x-master.form-input label="Jumlah Barang Keluar" type="number" name="jmlhbarangklr"
                    required="required" />
                @error("jmlhbarangklr")
                <x-master.form-error message="{{ $message }}" />@enderror

                <x-master.form-input label="Tgl. Barang Keluar" type="date" name="tglbarangklr" required="required" />
                @error("tglbarangklr")
                <x-master.form-error message="{{ $message }}" />@enderror

                <x-master.form-button cancel="{{ route('brg-keluar.index') }}" />
            </x-master.form>
        </x-master.card>
    </section>
</x-app-layout>
